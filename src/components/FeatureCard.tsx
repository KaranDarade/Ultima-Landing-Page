...
export default function FeatureCard({ index, title, number, items, isVideo, imageUrl, iconUrl, bgImageUrl }: FeatureCardProps) {
  ...
  return (
    <motion.div
      ...
      className="relative rounded-2xl overflow-hidden flex flex-col min-h-[320px] lg:min-h-0"
    >
      {isVideo ? (
        <>
          {imageUrl && (
        ...
        <>
          {(bgImageUrl || iconUrl) && (
            <img
              src={bgImageUrl || iconUrl}
              alt=""
              className="absolute inset-0 w-full h-full object-cover"
            />
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-[#212121] via-[#212121]/85 to-[#212121]/50 z-10" />
          <div className="relative z-20 p-5 md:p-6 flex flex-col h-full">
            <div className="flex items-center gap-3 mb-4">
              {iconUrl && (
                <img src={iconUrl} alt="" className="w-10 h-10 sm:w-12 sm:h-12 rounded object-cover ring-1 ring-white/10" />
              )}
              <div>
